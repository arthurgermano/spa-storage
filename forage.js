...
export default (options = {}) => {
  ...
  const IDX_DB_NAME = options.IDX_DB_NAME;
  const IDX_DB_STORE = options.IDX_DB_STORE;
...
const getItem = async (key) => {
  try {
    ...
    return fromJSON(fromCipher(key, await LF.getItem(key)));
  } catch (error) {
    throw error;
  }
};
...
const setItem = async (key, value, time) => {
  try {
    ...
    await LF.setItem(key, toCipher(key, toJSON(value)));
  } catch (error) {
    throw error;
  }
};
...
const removeItem = async (key) => {
  try {
    await removeExpiredKeys();
    const item = fromJSON(fromCipher(key, await LF.getItem(key)));
...
const clearExpireKeys = async () => {
  try {
    const expire = fromJSON(
      fromCipher(EXPIRE_KEYS, await LF.getItem(EXPIRE_KEYS))
    );
...
const clearKeyList = async (keyList) => {
  try {
    ...
    if (expire.length > 0) {
      await LF.setItem(EXPIRE_KEYS, toCipher(EXPIRE_KEYS, toJSON(expire)));
    } else {
      await LF.removeItem(EXPIRE_KEYS);
    }
  } catch (error) {
